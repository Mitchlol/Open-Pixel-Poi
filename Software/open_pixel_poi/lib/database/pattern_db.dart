...
class PatternDB {
  ...
  Future<List<img.Image>> getRepeatingImgImages(List<DBImage> dbImages) async {
    ...
    for (var dbImage in dbImages) {
      ...
      for (var pixel in imgimage) {
        pixel.r =
            dbImage.bytes[(((pixel.y % dbImage.height) * 3) + 0) + ((pixel.x % dbImage.count) * (dbImage.height) * 3)];
        pixel.g =
            dbImage.bytes[(((pixel.y % dbImage.height) * 3) + 1) + ((pixel.x % dbImage.count) * (dbImage.height) * 3)];
        pixel.b =
            dbImage.bytes[(((pixel.y % dbImage.height) * 3) + 2) + ((pixel.x % dbImage.count) * (dbImage.height) * 3)];
      }
      imgimages.add(imgimage);
    }
    return imgimages;
